function [] = Simulation_post_process(sensor_data, mode)
...
pixel_size_um = 20;             % µm per pixel
detector_size_um = 20;         % Detector diameter (µm)
step_size_um = 20;             % Detector step size (µm)
...
fc = 3e6; bw = 4e6;
bpFilt = designfilt('bandpassfir', 'FilterOrder', 400, ...
    'CutoffFrequency1', fc - bw/2, ...
    'CutoffFrequency2', fc + bw/2, ...
    'SampleRate', Fs, ...
    'Window', 'flattopwin');

for x = 1:size(sensor_data_3D,1)
    for y = 1:size(sensor_data_3D,2)
        signal = squeeze(sensor_data_3D(x,y,:));
        sensor_data_3D(x,y,:) = single(filtfilt(bpFilt, double(signal)));
    end
end
...
sensor_data_flat = reshape(sensor_data_input, [], N);  % [sensors x time]
...
% % % Plot PCA spectrum
hold on
plot(freq_axis, filtered_signal/max(filtered_signal), 'LineWidth',2);
xlabel('Frequency (Hz)');
